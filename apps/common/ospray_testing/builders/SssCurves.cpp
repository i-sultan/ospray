...
struct SssCurves : public detail::Builder
{
  SssCurves() = default;
  ~SssCurves() override = default;
...
cpp::Group SssCurves::buildGroup() const
{
  ...
  if (rendererType == "pathtracer") {
    ...
    glassMaterial.commit();
    model.setParam("material", glassMaterial);
  } else if (rendererType == "mis_pathtracer") {
    cpp::Material sssMaterial(rendererType.c_str(), "sss");
    sssMaterial.setParam("enableCoat", true);
    sssMaterial.setParam("roughness", 0.2f);
    sssMaterial.setParam("albedoColor", vec3f(0.9f, 0.9f, 1.f));
    sssMaterial.setParam("translucencyColor", vec3f(0.9f, 0.5f, 0.5f));
    sssMaterial.setParam("translucencyDepth", 0.005f);
    sssMaterial.setParam("sssAnisotropy", 0.2f);
    model.setParam("material", sssMaterial);
  }
...
OSP_REGISTER_TESTING_BUILDER(SssCurves, sss_curves);
